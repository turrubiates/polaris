            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form action="{{ route("admin.movimientos-bancarios.store") }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('referencia_id') ? 'has-error' : '' }}">
                            <label for="referencia">{{ trans('cruds.movimientosBancario.fields.referencia') }}</label>
                            <select name="referencia_id" id="referencia" class="form-control select2">
                                @foreach($referencias as $id => $referencia)
                                    <option value="{{ $id }}" {{ (isset($movimientosBancario) && $movimientosBancario->referencia ? $movimientosBancario->referencia->id : old('referencia_id')) == $id ? 'selected' : '' }}>{{ $referencia }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('referencia_id'))
                                <p class="help-block">
                                    {{ $errors->first('referencia_id') }}
                                </p>
                            @endif
                        </div>
                        <div class="form-group {{ $errors->has('descripcion') ? 'has-error' : '' }}">
                            <label for="descripcion">{{ trans('cruds.movimientosBancario.fields.descripcion') }}</label>
                            <input type="text" id="descripcion" name="descripcion" class="form-control" value="{{ old('descripcion', isset($movimientosBancario) ? $movimientosBancario->descripcion : '0') }}">
                        ...
                        <div class="form-group {{ $errors->has('codigo_de_transaccion') ? 'has-error' : '' }}">
                            ...
                            <input type="text" id="codigo_de_transaccion" name="codigo_de_transaccion" class="form-control" value="{{ old('codigo_de_transaccion', isset($movimientosBancario) ? $movimientosBancario->codigo_de_transaccion : '0') }}">
                        ...
                        <div class="form-group {{ $errors->has('descripcion_detallada') ? 'has-error' : '' }}">
                            ...
                            <input type="text" id="descripcion_detallada" name="descripcion_detallada" class="form-control" value="{{ old('descripcion_detallada', isset($movimientosBancario) ? $movimientosBancario->descripcion_detallada : '0') }}">
