...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.presupuestoAnual.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.presupuesto-anuals.update", [$presupuestoAnual->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('tipo') ? 'has-error' : '' }}">
                ...
                @foreach(App\PresupuestoAnual::TIPO_RADIO as $key => $label)
                    <div>
                        <input id="tipo_{{ $key }}" name="tipo" type="radio" value="{{ $key }}" {{ old('tipo', $presupuestoAnual->tipo) === (string)$key ? 'checked' : '' }}>
